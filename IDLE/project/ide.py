...
font_box.current(font_tuple.index('Courier New'))                                             # initializing the default font
...
# bold button
bold_icon = tk.PhotoImage(file=r"D:\CBIT\DIP_PROJ\IDLE\project\bold.png")                   # creating a Bold Icon for the Bold Button
...
# italic button
italic_icon = tk.PhotoImage(file=r"D:\CBIT\DIP_PROJ\IDLE\project\italic.png")               # creating a Italic Icon for the Italic Button
...
# underline button
underline_icon = tk.PhotoImage(file=r"D:\CBIT\DIP_PROJ\IDLE\project\underline.png")         # creating a Underline Icon for the Underline Button
...
textarea = Text(editFrame, font=('Courier New', font_size_now, 'bold'),yscrollcommand=scrollbar.set, wrap="none", xscrollcommand=scrollbar1.set)
...
def about(event=None):
    os.startfile(r'"D:\CBIT\DIP_PROJ\IDLE\project\about.pyw"')  # Starting a file to show the about PyBook, only extension with (.pyw)
...
def openfile(event=None):
    ...
    root.title(f" PyBook - {path}")                            # This will be Visible as IDE name + File path on top of the Window
    if path != '':
        file = open(path, 'r')                                 #  opening the file in read mode
        data = file.read()                                     # storing in the data 
        textarea.delete(1.0, END)                              # if the texts exits in the textarea ,it will delete the text 
        textarea.insert(1.0, data)                             # And then it inserts the opened file into the textarea
        file.close()                                           # Closing a file
    ...
    try:
        if event:
            ...
            if textarea.selection_get():
                ...
                root.clipboard_append(selected)           # and then append the text
    except TclError:
        pass
...
view_menu.add_checkbutton(label='Tool Bar',   onvalue=False, offvalue=1,     variable=show_toolbar,    compound=tk.LEFT,command=hide_toolbar)
view_menu.add_checkbutton(label='Status Bar', onvalue=1,     offvalue=False, variable=show_status_bar, compound=tk.LEFT,command=hide_status_bar)
...
p1 = PhotoImage(file = r"D:\CBIT\DIP_PROJ\IDLE\project\icon.png")
